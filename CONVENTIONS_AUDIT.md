...
- **Existing measurements** from KNOWN_ISSUES.md, marked "KI-n" for a defect and "KI Ln" for a limitation. Those IDs are permanent and never reused, so a reference here stays valid after the list is reordered or an entry is fixed. References written before 2026-09-22 used the position in that list on the day; they were converted to IDs on 2026-09-22, when the positions and the IDs still agreed.
...
| 2 | Consistent definition of "core operating" EBITDA across the comp set and the exit multiple (Comps 4; DCF 12) | Hard rule | 22 valued (SBC >5% of EBITDA) | Arm, SBC 47.6% of EBITDA | exit-multiple value −36.3%; EV/EBITDA method overstated 90.9%; fixed after this audit: EBITDA is before D&A only, the basis the peer multiples are on (KI L21) |
...
| 4 | Depreciation schedule: existing PP&E vs capex vintages, useful lives, historical plausibility check (Dep 6, 8) | Hard rule / rule of thumb | 15 of 82 valued move >5% | Amazon −24.2%; Toyota Tokyo +23.8% | the rate is now read from filed depreciation and charged on the assets in service, not on the year's capital spending (both fixed after this audit: KI L28); vintages and lives are not |
...
| 13 | Other income sits in its own section with a documented above/below-EBIT decision; non-recurring items separated (IS 11, 14, 17) | Hard rule / rule of thumb | 28 of 64 valued with other non-operating income above 10% of operating profit | Alibaba, 133% | **fixed after this audit** (2026-09-24): other non-operating income is forecast rather than set to nil, so the tax rate is applied to the base it was measured on. Non-recurring items are still not separated from it, which is why the projection takes whichever of the last year and the median is closer to nil |
| 14 | Capex projected from guidance, not a percentage of revenue alone (CF 6) | Rule of thumb | every derived company | — | **fixed after this audit** (KI L30): capex is replacement plus growth, and the asset base now holds its own capital intensity — more than 10% away from the company's reported average fell from 53 of 70 to 27 of 69; 31 of 85 valued companies moved more than 5% |
...
| CF 6 | Capex off guidance, cross-checked with % of revenue | Not applicable in part; **the rest fixed after this audit** | Guidance needs management commentary, which the site does not read: not applicable, and the cross-check has nothing to cross-check against. The half that can be kept was, on 2026-09-22: capex is no longer a percentage of revenue in isolation but replacement of what wears out (equal to depreciation) plus this company's own net PP&E to revenue ratio applied to each year's increase in revenue (`deriveModel.js:1177-1187`, `model.js` PP&E schedule; KI L30). It was the average capex % of revenue across the reported years, capped at 40%, with no second reference, which softened lumpiness but let the asset base drift away from the plant the company carries. A planned step-change such as an AI data-centre build-out is still not visible to it. Failures #14. |
...
| Dep 6 | Existing PP&E separated from each capex vintage | **Partly followed** | Still one pool, but depreciation is charged on the assets in service — opening net PP&E plus half the year's additions — rather than on that year's capital spending (fixed after this audit: `model.js`, PP&E schedule; KI L28). Vintages and useful lives remain absent, which is the rest of this rule. Amortisation of intangibles is separated and runs off (`model.js:273`). Failures #4. |
...
| # | Item | Verdict | Reason |
|---|---|---|---|
| WC 1 | Narrow definition: ex cash, ex interest-bearing debt | Partly followed | Cash and debt are excluded. But deferred tax assets, other assets and other non-current liabilities also move through the working capital block and unlevered free cash flow (`model.js:154-208`, `:1062-1073`), and other current assets could hold marketable securities, which are now fetched and taken out of it (KI L22). Failures #12. |
...
| BS 2 | CF drives BS (not BS differencing) | Partly followed | Forecast years are CF-driven. Reported-year cash flows in the workbook are derived by differencing filed balance sheets, which the Cash Flow sheet says in a note (`excelSheets.ts:720-723`; KI-7). |
...
| Debt 2 | Every debt instrument has a section | **Not followed** | Fixed after this audit: short-term borrowings, the current portion of long-term debt and finance lease liabilities are fetched and carried in one borrowings line (KI L24). Operating leases under US GAAP are reported and not netted, with the reason. The model still tracks one debt stock plus a revolver rather than a section per instrument (`model.js:482-508`). Failures #5. |
...
| Debt 6 | Interest on average balance × rate | Partly followed | Workbook: opening balance with the switch off (the default), average with it on (`excelExport.ts:629-680`). Site engine: a fixed dollar amount on the last reported borrowings at the company's own filed rate (`deriveModel.js`, `costOfDebtRate`). Fixed after this audit: the rate was a flat 4.5% for every company (KI L25). |
...
| DCF 5 | Unlevered tax = EBIT × rate | Followed | `M.ebit[i] * (1 - M.taxRate[i])`. The rate is the filed effective rate, whose denominator the forecast now also carries (fixed after this audit, 2026-09-24). What the filing cannot separate — the tax borne by the non-operating half of that denominator — is bounded per company in DATA_CONSTRAINTS.md. |
...
| DCF 7 | WACC on market-value weights, after-tax debt | **Fixed after this audit** | Cost of debt is tax-effected, equity is at market value, and the debt weight is now GROSS debt at book value, `grossDebt / (marketCap + grossDebt)` (`model.js`, `computeWACC`). It was net debt, so 27 of the 63 valued companies carried a negative debt weight, an equity weight above 100% and a WACC above their own cost of equity; now none do. The beta relevering carries gross debt too (KI L23). The workbook still hard-codes the engine's weights as inputs (`excelExport.ts` rows 24-25). Failures #7. |
...
| DCF 12 | Both TV methods, discounted consistently | Followed | Both are built and discounted at the same WACC and final-period factor (`model.js:1094-1143`; `excelExport.ts` rows 33-44). The exit multiple is a flat 12x for derived companies (`deriveModel.js:776`), which is why the perpetuity value is the one used wherever a single figure is needed (KI L26). |
...
| DCF 15 | EV − net debt − NCI − preferred, ÷ diluted shares | **Not followed** | `equityValue = enterpriseValue − netDebt` only (`model.js:1169-1170`; `excelExport.ts` rows 47-51). No NCI (up to 48.1%) and no preferred at the time of this audit; both have since been deducted. Cash now includes short-term marketable securities (KI L22). Finance leases are included and operating leases deliberately are not (KI L24). Failures #3 and #5. |
...
| Comps 4 | Non-recurring items reclassified consistently across the set | **Fixed after this audit** | the subject's EBITDA no longer adds SBC back, matching the basis the peers' provider computes (`model.js`, `S.ebitda`; KI L21). Identifying non-recurring items would not be applicable (research). Failures #2. |
