...
1. **NEXT — Remaining `KNOWN_ISSUES.md` method entries.** The top of that file is now two of the three found reviewing the capital-heavy companies on 2026-09-22: most of a valuation is the terminal year (KI-12), and forecast capital spending assumes revenue measures the size of the plant (KI-11). The third, the growth path stepping into the terminal rate (KI-13), was fixed on 2026-09-23 together with the fiscal year end (KI-14); that closed the step change half of KI-12 and the extreme cases of KI-11, and left the dominance of the terminal value untouched. KI-11 was then attacked directly: six bounds on capital spending were measured and rejected, which relocated it to the revenue line, and it was fixed there on 2026-09-23. A company whose revenue fell while the plant that produces it rose is now refused, which closed the worse tail and took nothing else with it; a year whose growth was bought rather than earned is warned about, because the filing does not say how much of it was bought. KI-12 was then settled: the terminal value being three quarters of a valuation is the arithmetic of a five-year window, not a defect — 52 of 64 companies sit within five points of what a flat cash flow would give at the same rates — so it is now disclosed on every valuation and in the workbook rather than argued with, and what remains of KI-12 is the three companies far above that benchmark. KI-3 was fixed on 2026-09-24: the forecast now carries the other non-operating income the tax rate's denominator always included, so the rate is applied to the base it was measured on, and the payout ratio's denominator was corrected the same way. What is left of KI-11 is the middle of that family — Shell and BP, whose plant fell but far slower than their revenue — and catching them needs a threshold on the magnitude of the divergence that has not been found. KI-2 was fixed the same day: the valuation date is now the last reported balance sheet date, which is the date the equity bridge already used, and the convention is mid-year and stated. The same filings give the same answer on whatever day they are opened, which the suite checks. Eight entries stand, and the list is meant to reach zero. Entries there carry permanent IDs from 2026-09-22: `KI-4` means the same defect for good, and fixing one no longer renumbers the rest.
