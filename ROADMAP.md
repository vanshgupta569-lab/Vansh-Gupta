...
1. **NEXT — Remaining `KNOWN_ISSUES.md` method entries.** The top of that file is now the three found reviewing the capital-heavy companies on 2026-09-22: most of a valuation is the terminal year and the terminal year is a step change from the forecast (KI-12), revenue growth is a clamped trailing average capitalised at 2.5% forever (KI-13), and forecast capital spending assumes revenue measures the size of the plant (KI-11). They are method questions about the shape of the DCF rather than missing data, and KI-12 is the one that decides how much the other two matter. Entries there carry permanent IDs from 2026-09-22: `KI-4` means the same defect for good, and fixing one no longer renumbers the rest.
